...
- A Java 26 or later JDK (packages vary, use Google/DuckDuckGo/etc.).
  - [Adoptium](https://adoptium.net/) has builds for most operating systems.
  - Paper requires JDK 26 to build, however, makes use of Gradle's
    [Toolchains](https://docs.gradle.org/current/userguide/toolchains.html)
    feature to allow building with only JRE 21 or later installed. (Gradle will
    automatically provision JDK 26 for compilation if it cannot find an existing
    installation).
...
```console
# docker run -it -v "$(pwd)":/data --rm eclipse-temurin:26-jdk bash
Pulling image...
